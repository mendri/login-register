        ...
        <section className="h-screen w-full flex justify-center items-center">
            <form className="w-1/2 h-4/5 flex flex-wrap justify-center items-center border-4 rounded-md shadow-xl" onClick={(e) => e.preventDefault()}>
                ...
                <div className="w-full flex flex-col justify-center items-center">
                    <button id="login-input-btn" disabled={!IS_THE_FORM_CORRECTLY_TYPED} onClick={handleLoginBtn} className="w-4/12 h-8 border rounded-md bg-teal-500 text-neutral-50 font-bold" data-testid="login-input-btn">Registrar</button>
                    <button onClick={() => NAVIGATE("/login")} className="w-4/12 h-8 mt-4 border rounded-md bg-orange-500 text-neutral-50 font-bold" data-testid="register-input-btn">Logar</button>
                </div>
            </form>
        </section>
    );
